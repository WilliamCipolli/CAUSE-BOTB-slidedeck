histoqq <- function(data, color = FALSE, square=FALSE, dist = "norm", height.scale = 1, ...) {
  ...
  if (length(user.params) == 0) {
    if (dist == "unif") { # not well supported
      ...
      tryCatch({
        ...
        warning("MASS::fitdistr could not automatically estimate parameters for the '", fit.name, 
                "'distribution. Please check your specification and ensure any necessary libraries are loaded. Error: ", e$message)
      })
    }
  }
  ...
  theoretical.density <- data.frame(val = plot.range) |>
    ...
    mutate(
      ...
      x.base = (val - intercept) / slope - (if(is.discrete) (true.w / 2) * vx else 0),
...
# histoqq(rnorm(200), color = TRUE)
# histoqq(rnorm(200), color = FALSE)

# histoqq(rt(200, df=3), color = TRUE)

# library(VGAM)
# histoqq(rlaplace(200), color = TRUE)

# histoqq(runif(200), color = TRUE)
# histoqq(runif(200), color = TRUE, dist="unif")

# histoqq(rexp(200), color = TRUE)
# histoqq(rexp(200), color = TRUE, dist = "exp", rate=1)

# histoqq(rbeta(200,10,1), color = TRUE)
# histoqq(rbeta(200,10,1), color = TRUE, dist="beta", shape1=10, shape2=1)

# # Discrete works too!
# histoqq(rbinom(200, size=20, prob=0.35), dist="binom", size=20, prob=0.35)
# histoqq(rbinom(200, size=2, prob=0.35), dist="binom")
# histoqq(rbinom(200, size=2, prob=0.35), dist="pois", color = TRUE)
# histoqq(rpois(200, lambda=1), dist="pois", color = TRUE)
# histoqq(rpois(200, lambda=1), dist="pois", lambda=1)
